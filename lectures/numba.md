...
然而，正如 {ref}`在那次讲座中所讨论的 <numba-p_c_vectorization>`，传统的向量化方案（例如在 MATLAB、Julia 和 NumPy 中）存在一些弱点。
...
事实上，下一次及所有后续运行会更快，因为函数已经被编译并存储在内存中：
...
这使它能够生成高效的本地机器码，而无需调用 Python 运行时环境。
