...
JAX 的设计者选择将数组设为不可变的，因为 JAX 使用函数式编程风格，我们将在下文中讨论这一点。
...
JAX 将函数表示为计算图，然后对其进行编译或变换（例如，求导）。

这些计算图描述了给定的一组输入如何被转换为输出。

它们在构造上就是纯粹的。

JAX 使用函数式编程风格，使得用户构建的函数能够直接映射到 JAX 所支持的图论表示中。
...
下图说明了 `split` 如何从单个根生成密钥树，每个密钥生成独立的随机抽取。
...
我们在 {ref}`上面 <jax_speed>` 将 `cos` 应用于大型数组时，已经见识了 JAX 的 JIT 编译器结合并行硬件的强大能力。
