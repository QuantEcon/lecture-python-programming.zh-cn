...
让我们从函数开始，它将 `np` 替换为 `jnp` 并添加了 `jax.jit`。
...
这很好——但我们还是遗漏了一些速度提升的空间！

首先，上面的代码在取最大值之前会计算完整的二维数组 `f(x,y)`，这会产生额外开销。

其次，`jnp.max` 调用位于 JIT 编译函数 `f` 之外，因此编译器无法将这些操作融合为单个内核。

我们可以通过将求最大值操作移到内部并将所有内容包装在单个 `@jax.jit` 中来解决这两个问题：
...
由于我们将求最大值操作移到内部，我们永远不会构建完整的二维数组 `f(x,y)`，从而节省了更多内存。
