...
现在让我们退一步，总结一下各方面的权衡取舍。

对于**向量化操作**，JAX 是最强的选择。

得益于 JIT 编译以及在 CPU 和 GPU 上的高效并行化，JAX 的速度与 NumPy 相当甚至超越 NumPy。

`vmap` 变换可以减少内存使用，并且通常比基于 meshgrid 的传统向量化产生更清晰的代码。

此外，JAX 函数支持自动微分，我们将在 {doc}`autodiff` 中进一步探讨。

对于**顺序操作**，Numba 具有明显优势。

代码自然易读——只需在 Python 循环上添加一个装饰器——性能也十分出色。

JAX 可以通过 `lax.scan` 处理顺序问题，但语法不够直观，且对于纯顺序工作的性能提升微乎其微。

尽管如此，`lax.scan` 有一个重要优势：它支持对循环进行自动微分，而 Numba 无法做到这一点。

如果你需要对顺序计算进行微分（例如，计算轨迹对模型参数的敏感性），尽管语法不够自然，JAX 仍是更好的选择。

在实践中，许多问题同时涉及两种模式。

一个实用的经验法则是：新项目默认选择 JAX，尤其是当硬件加速或可微分性可能有用时；而当你有需要快速且易读的紧密顺序循环时，则选用 Numba。
