...
我们{ref}`已经介绍过<iterating_version_1>`在 Python 中的迭代。
...
从形式上讲，*迭代器*是一个具有 `__next__` 方法的对象。

例如，文件对象就是迭代器。

为了说明这一点，让我们再看一下{ref}`美国城市数据<us_cities_data>`，该数据将在以下单元格中写入当前工作目录
...
我们可以看到文件对象确实具有 `__next__` 方法，调用该方法会返回文件中的下一行。

`next` 方法也可以通过内置函数 `next()` 来访问，该函数直接调用此方法
...
* 执行代码块
* 重复执行，直到发生 `StopIteration` 错误

现在你明白了这个看起来很神奇的语法是如何工作的
...
你已经知道我们可以在 `for` 循环中将 Python 列表放在 `in` 的右侧
...
从形式上讲，如果一个对象可以使用内置函数 `iter()` 转换为迭代器，则该对象是可迭代的。
...
`*` 和 `**` 是方便且广泛使用的工具，用于解包列表和元组，并允许用户定义可以接受任意数量参数作为输入的函数。

在本节中，我们将探讨如何使用它们并区分它们的使用场景。

### 解包参数

当我们对参数列表进行操作时，我们通常需要在将参数传递给函数时，将列表的内容提取为单独的参数而不是一个集合。

幸运的是，`*` 运算符可以帮助我们将列表和元组解包为函数调用中的[*位置参数*](pos_args)。

为了具体说明，请参考以下示例：

不使用 `*` 时，`print` 函数会打印一个列表
...
而使用 `*` 时，`print` 函数会打印各个元素，因为 `*` 将列表解包为单独的参数

```{code-cell} python3
print(*l1)
```

使用 `*` 将列表解包为位置参数，等价于在调用函数时单独定义它们

```{code-cell} python3
print('a', 'b', 'c')
```

但是，如果我们想要再次复用这些参数，`*` 运算符会更加方便
...
`**` 常用于当我们有许多想要复用的关键字参数时。

例如，假设我们想要使用相同的图形设置绘制多个图表，这可能涉及重复设置许多图形参数，这些参数通常使用关键字参数来定义。
...
总结一下，当 `*list`/`*tuple` 和 `**dictionary` 被传入*函数调用*时，它们会被解包为单独的参数而不是一个集合。
...
总的来说，`*args` 和 `**kargs` 用于*定义函数*时，它们使函数能够接受任意数量的输入。

区别在于，带有 `*args` 的函数能够接受任意数量的*位置参数*，而 `**kargs` 则允许函数接受任意数量的*关键字参数*。
...
Python 是一种*动态类型*语言，这意味着你无需声明变量的类型。

（参见我们{doc}`之前关于 <need_for_speed>`动态类型与静态类型的讨论。）

然而，Python 支持可选的**类型提示**（也称为类型注解），允许你指明变量、函数参数和返回值的预期类型。

类型提示从 Python 3.5 开始引入，并在后续版本中不断演进。
此处展示的所有语法均适用于 Python 3.9 及更高版本。

```{note}
类型提示在*运行时会被 Python 解释器忽略*——它们不影响代码的执行方式。它们纯粹是信息性的，作为面向人类和工具的文档。
```
...
对于 Python 新手来说有一点很重要：类型提示在运行时*不会被强制执行*。

如果你传入了"错误"的类型，Python 不会报错：
...
提示声明的是 `int`，但 Python 愉快地接受了 `float` 参数并返回 `4.2`——同样不是 `int`。

这是与 C 或 Java 等静态类型语言的关键区别，在那些语言中，类型不匹配会导致编译错误。

### 为什么使用类型提示？

如果 Python 忽略它们，为什么还要使用？

1. **可读性**：类型提示使函数签名具有自文档化的效果。读者可以立即了解函数期望接收和返回什么类型。
2. **编辑器支持**：VS Code 等集成开发环境利用类型提示提供更好的自动补全、错误检测和内联文档。
3. **错误检查**：[mypy](https://mypy.readthedocs.io/) 和 [pyrefly](https://pyrefly.org/) 等工具通过分析类型提示，在*运行代码之前*捕获错误。
4. **大语言模型生成的代码**：大型语言模型经常生成带有类型提示的代码，因此理解其语法有助于你阅读和使用其输出结果。

### 科学 Python 中的类型提示

类型提示与{doc}`性能需求 <need_for_speed>`的讨论密切相关：

* [JAX](https://jax.readthedocs.io/) 和 [Numba](https://numba.pydata.org/) 等高性能库依赖于了解变量类型来编译高效的机器码。
* 尽管这些库在运行时推断类型，而非直接读取 Python 类型提示，但其*概念*是相同的——显式的类型信息能够实现优化。
* 随着 Python 生态系统的演进，类型提示与性能工具之间的联系预计将进一步加强。

目前，类型提示在日常 Python 编程中的主要价值在于*清晰性和工具支持*，随着程序规模的增长，这一价值将愈发显著。
...
您可能不会立即用到以下概念，但您会在其他人的代码中看到它们。

因此，您需要在 Python 学习的某个阶段理解它们。
...
装饰器是一种语法糖，虽然很容易避免使用，但已被证明非常流行。

说明装饰器的功能非常容易。

但解释*为什么*要使用它们则需要花费一些精力。
...
现在假设出现了一个问题：在随后的计算中，偶尔会有负数被传入 `f` 和 `g`。

如果您尝试一下，就会看到当这些函数被传入负数时，它们会返回一个名为 `nan` 的 NumPy 对象。

这代表"不是一个数字"（表示您正试图在一个未定义的点上求值一个数学函数）。

也许这不是我们想要的，因为它会导致其他难以在后续发现的问题。

假设我们希望程序在这种情况发生时立即终止，并给出一个有意义的错误信息。
...
重复使我们的代码变得更长、更难维护，因此是我们努力避免的事情。

在这里这不是什么大问题，但现在想象一下，不只是 `f` 和 `g`，我们有 20 个这样的函数需要以完全相同的方式修改。
...
为了理清逻辑，考虑当我们执行 `f = check_nonneg(f)` 时会发生什么。

这会调用函数 `check_nonneg`，并将参数 `func` 设置为 `f`。

现在 `check_nonneg` 创建了一个名为 `safe_function` 的新函数，它验证 `x` 为非负数，然后在其上调用 `func`（即与 `f` 相同）。
...
例如，如果有人正在阅读我们的代码并想了解 `f` 的工作原理，他们会寻找函数定义，即
...
假设这个类定义了变量 `miles` 和 `kms`，分别以英里和公里给出行驶距离。
...
我们可能遇到的一个潜在问题是，用户修改了其中一个变量而没有修改另一个
...
方法 `get_miles`、`set_miles`、`get_kms` 和 `set_kms` 定义了当您获取（即访问）或设置（绑定）这些变量时会发生什么

* 即所谓的"getter"和"setter"方法。

Python 内置函数 `property` 接受 getter 和 setter 方法并创建一个属性。

例如，在 `car` 作为 `Car` 的实例创建之后，对象 `car.miles` 就是一个属性。

作为一个属性，当我们通过 `car.miles = 6000` 设置其值时，它的 setter 方法会被触发——在本例中是 `set_miles`。
...
如今，通过装饰器使用 `property` 函数已经非常普遍。

下面是我们 `Car` 类的另一个版本，其功能与之前相同，但现在使用装饰器来设置属性
...
如需进一步了解，您可以参考[描述符文档](https://docs.python.org/3/howto/descriptor.html)。
